...
function placeSpecials(maze, SEED){
  ...
  const restCand = cellsAtDistanceRange(maze, START.x, START.y, 3, 7, used); const restroom = pickOne(rng0, restCand, used) || {x:(START.x+2)%WIDTH,y:(START.y+4)%HEIGHT}; used.add(keyOf(restroom));
